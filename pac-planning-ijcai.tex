...
\typeout{Safe Model-Free Planning}
...
%Roni, feel free to change the title. This errs on the side of bloodless accuracy. 
...
\begin{document}
	...
	\begin{abstract}
		...
		planning in a setting where there is no model of the agent's actions. 
	...
	%creating a planning domain model for an agent, and in general knowledge engineering, is notoriously hard. % Missing a connection between these two sentence. Maybe ``For example?''
	Lenat~\shortcite{lenat1995} gave a nice overview of the many challenges in knowledge engineering that were encountered during the CYC project.
	% Is this reference for general knowledge engineering or for planning?
	
	% Past work in a nutshell: either learn a model, which is hard computationally, or learn to plan directly. 
	A well-studied approach to address this is to learn a domain model by observing the agent's  interactions with the environment, and by guiding the agent towards performing exploratory actions. The problems that arise in such approaches, however, are frequently intractable~\cite{kearns1994,daniely2016}. An alternative approach
	that is frequently used in {\em reinforcement learning} is to skip the model-learning phase and directly learns how to act from observing the agent's past actions and observations~\cite[inter alia]{kearns2002}. In most prior work, the agent may fail to execute a planned action. When this occurs, the agent replans, possibly refining an underlying domain or action model it has learned. Thus, the agent learns from both positive and negative examples. 
	
	
	
	%Thus, a well-studied approach is to learn a domain model by observing the agent's  interactions with the environment. , and by guiding the agent towards performing exploratory actions. A popular approach to planning without a domain model in {\em reinforcement learning} even skips the model-learning phase and directly learns how to act from observing the agent's past actions and observations, e.g.~\cite{kearns2002}. 
    ...
    % Roni: these are two different motivations for our work: one is that the robot is too expensive and fragile to allow failures, and the other is that replanning during execution is not allowed.	
	Consider, for example, a team of nano-bots deployed inside a human body for medical target identification and drug delivery~\cite{cavalcanti2007nanorobot}. Re-planning is likely not possible in such nano-bots, and, of course, failing to cure the disease is undesirable. Thus, the planning task we focus on in this paper is how to find a that is {\em safe}, i.e., is guaranteed to achieve the goal, in a setting where a domain model is not available. We call this problem the {\em safe model-free planning problem}. 
	
		%Furthermore, an agent may have limited computational capabilities, and thus might not have the capability to re-plan after the plan it has tried to execute has failed. [[Roni: this is an alternative motivaiton, not another assumpiton we are making, so I differentiated the two]]
	...
	%\roni{I moved this part from earlier in the intro to here for several reasons: (1) it is too technical to "hit" the reader with it so early, and (2) we want to get the reader "faster" to our problem and approach. I hope this is OK with you.]]
	This positive result comes in contrast to the hardness of other model-learning tasks. 
...
The key difference between the model learning we propose and these hardness results is in that limit our attention to the learning of STRIPS domain models in a PAC (``Probably Approximately Correct'') sense: we do not aim to learn an accurate action model, only one that is sufficient for finding a safe plan in most cases. We introduce and desribe this PAC-like setting in Section~\ref{problemdef-sec}.%
	%\footnote{Actually, in general STRIPS planning is also known to be intractable~\cite{bylander1994,erol1995}, so our difficulties do not end once we have obtained a domain model. Nevertheless, broad families of STRIPS planning problems can be solved efficiently in practice.}[I removed the poly-time argument, so to avoid this footnote}
	 
	 
	% We will give provably polynomial-time algorithms for the problem of learning a domain model, so obviously none of these problems poses an insurmountable barrier; nevertheless, we wish to highlight the many pitfalls that one may encounter. 	For example, learning to predict labels computed by finite-state machines~\cite{kearns1994} or even DNF formulas~\cite{daniely2016} is believed to be computationally intractable. Thus, we cannot hope to predict the values that fluents will take merely on the basis of the fact that these can be computed by a finite-state machine or a DNF. 
	%\roni{Intractable in terms of the sample complexity, right?}     \brendan{No, the above is computational -- see clarification}	\roni{The relation between this example and our problem is not easy to follow for a classical planning person like me.} 	\brendan{I tried to clarify the connection -- let me know if it's OK}\roni{Thi is great, I now understand - thanks!}.

	
	
	%\MEMO{I think it is better to say something more accurate than ``quickly''. How do you say in English taht something grows in a $n\cdot logn$ manner? super-linearly?}
	...
	\section{Learning to Perform Safe Planning}
...
%		Finally, we argue that the action model used by our conservative model-free planner is $\epsilon$-adequate with probability at least $1-\delta/2$. 
		...
		Clearly, $BAD$ has size of at most $d^{|\mathcal{X}|\cdot |A|}$. 
...
The first row shows the probabilities 
	...
	Similarly, most prior work on learning a domain model in general 
	or a STRIPS action model from observed trajectories, 
	such as ARMS~\cite{yang2007learning} and LOCM~\cite{cresswell2013acquiring},
	also involve some form of trial-and-error as well, interacting with environment to refine the learned model~\cite{mourao2012learning,wang1994learning,wang1994learning,walsh2008efficientLearning,levine2006explanation,jimenez2013integrating}. 
