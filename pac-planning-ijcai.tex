...
\begin{document}
	...
	\caption{An example of creating $F(\Pi_{\mathcal{T}})$ from observed trajectories in a simple logistic-like domain.}
	\label{fig:example}
	\end{figure*}
	...
	the action model learned using all three trajectories. As can be observed, given only $T_1$ we do not have any knowledge of many actions such as Pick$_A$, Pick$_C$, etc. Also, the preconditions learned for the actions Move$_{A,B}$ 
