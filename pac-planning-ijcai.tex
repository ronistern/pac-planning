...
\begin{document}
	...
	that is frequently used in {\em reinforcement learning} is to skip the model-learning phase and directly learn how to act from observing the agent's past actions and observations, and by guiding the agent towards performing exploratory actions~\cite[inter alia]{kearns2002}. In most prior work, the agent may fail to execute a planned action. When this occurs, the agent replans, possibly refining an underlying domain or action model it has learned. Thus, the agent learns from both positive and negative examples. 
